...
/**
 * Sanitizes a font family name into a valid enum key.
...
async function writeFile(filePath: string, content: string): Promise<void> {
  try {
    ...
    logger.log(`✅ Successfully wrote: ${path.basename(filePath)}`);
  } catch (error) {
...
/**
 * Generates the content for the font family enum file (fontFamily.enum.ts).
 */
function createFontFamilyEnumContent(allFonts: GoogleFontItem[]): string {
...
/**
 * Generates the content for the font family map file (fontFamily.map.ts).
 */
function createFontFamilyMapContent(allFonts: GoogleFontItem[]): string {
  const mapEntries = allFonts
    .map(font => {
      const enumKey = sanitizeFamilyForEnum(font.family);
      return `  [FontFamily.${enumKey}]: ${objectToString(font, 2)},`;
    })
    .join("\n");
...
export const googleFontFamilyMap: Record<FontFamily, GoogleFontItem> = {
${mapEntries}
};
`;
}
...
export async function generateFontSubsetFiles(
  ...
  // 6. Collect all unique fonts sorted by family name
  logger.log("Collecting all unique fonts for family enum/map...");
  const allFonts = Array.from(
    new Map(
      Array.from(subsetMap.values())
        .flat()
  ...
  logger.log("\n🎉 Font subset file generation complete.");
}
