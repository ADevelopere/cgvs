...
export interface TemplateVariablesContext {
    ...
    reorderVariables: (variableIds: number[]) => Promise<void>;
    moveVariableUp: (variableId: number) => Promise<void>;
    moveVariableDown: (variableId: number) => Promise<void>;
}
...
export function TemplateVariablesProvider({
    ...
    const fetchVariables = useCallback(async (templateId: number) => {
        ...
        try {
            ...
            const sortedVariables = [...response.data].sort(
                (a, b) => a.order - b.order,
            );
    ...
    const deleteVariable = useCallback(async (variableId: number) => {
        try {
    ...
    const reorderVariables = useCallback(async (variableIds: number[]) => {
        try {
    ...
    const moveVariableUp = useCallback(
        async (variableId: number) => {
            ...
            [newVariableIds[currentIndex - 1], newVariableIds[currentIndex]] = [
                newVariableIds[currentIndex],
                newVariableIds[currentIndex - 1],
            ];

            await reorderVariables(newVariableIds);
        },
    ...
    const moveVariableDown = useCallback(
        async (variableId: number) => {
            ...
            if (currentIndex === -1 || currentIndex >= variables.length - 1)
