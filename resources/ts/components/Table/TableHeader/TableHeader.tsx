...
const TableHeader: React.FC<{
  ...
  // State to track which columns have their filter UI open
  ...
  const [activeOptionsColumn, setActiveOptionsColumn] = useState<string | null>(
    null
  );
  ...
  const handleInlineFilterIconClick = useCallback(
    (e: React.MouseEvent, columnId: string) => {
      e.stopPropagation(); // Prevent triggering sort

      // Toggle the column in openFilterColumns
      ...
      // Initialize filter value if not set
      if (tempFilterValues[columnId] === undefined) {
        const currentFilter = filters[columnId];
        let filterValue = "";
    ...
    [filters, tempFilterValues, setTempFilterValues]
  );
  ...
  const getFilterIconClickHandler = useCallback(
    (column: (typeof columns)[0]) => {
      ...
      if (column.type === "text") {
        ...
        return handleInlineFilterIconClick;
      }
    },
    [
      ...
      handleInlineFilterIconClick,
      serverOperationMode,
    ]
  );

  // Add onServerFiltersChange function
  // const onServerFiltersChange = (filters: FilterClause<any, any>[]) => {
    ...
    >
      ...
      >
        ...
        {columns.map((column) => {
          ...
          // Check if this column has an active server filter when in server mode
          const hasActiveServerFilter = useMemo(() => {
            return (
              serverOperationMode &&
              column.serverFilterable &&
              activeServerFilters.some((f) => f.columnId === column.id)
            );
          }, [
            serverOperationMode,
            column.serverFilterable,
          ...
          return (
            <ColumnHeaderCell
              ...
              onInlineFilterIconClick={handleInlineFilterIconClick}
              onPopoverFilterIconClick={handlePopoverFilterIconClick}
              onTextFilterIconClick={getFilterIconClickHandler(column) as any}
      ...
      {/* Render the inline filter row if in server mode with inlineHeaderRow UI */}
      <InlineFilterRow />
      ...
      <ColumnOptionsMenu
        ...
        columnLabel={
          columns.find((col) => col.id === activeOptionsColumn)?.label || ""
        }
        isPinnedLeft={pinnedColumns[activeOptionsColumn || ""] === "left"}
        isPinnedRight={pinnedColumns[activeOptionsColumn || ""] === "right"}
      ...
      <FilterPopover
        ...
        columnLabel={
          columns.find((col) => col.id === activeFilterColumn)?.label ?? ""
        }
        value={
          activeFilterColumn ? tempFilterValues[activeFilterColumn] ?? "" : ""
        }
        hasActiveFilter={
          !!activeFilterColumn &&
          (!!filters[activeFilterColumn] ||
            (serverOperationMode &&
              activeServerFilters.some(
      ...
      {activeTextFilterColumn && (
        <TextFilterPopover
          ...
          columnLabel={
            columns.find((col) => col.id === activeTextFilterColumn)?.label ||
            ""
          }
      ...
      {activeNumberFilterColumn && (
        <NumberFilterPopover
          ...
          columnLabel={
            columns.find((col) => col.id === activeNumberFilterColumn)?.label ||
            ""
          }
          activeFilter={getActiveNumberFilter(activeNumberFilterColumn)}
        />
      )}
      ...
      {activeDateFilterColumn && (
        <DateFilterPopover
          ...
          columnLabel={
            columns.find((col) => col.id === activeDateFilterColumn)?.label ||
            ""
          }
