...
import React, { CSSProperties, FC, ReactNode, TouchEvent, useCallback, useEffect, useMemo, useRef, useState } from "react";
...
const EditorPane: FC<EditorPaneProps> = ({
  ...
  const [paneSizes, setPaneSizes] = useState<number[]>([0, 0, 0]);

  // Refs to store previous state for comparison
  // Store individual sizes of hidden panes
  const previousPaneSizesRef = useRef<{ first: number | null; third: number | null }>({ first: null, third: null });
  const previousVisibilityRef = useRef({ first: firstPane.visible, third: thirdPane.visible });
  ...
  useEffect(() => {
    ...
    const currentVisibility = { first: firstPane.visible, third: thirdPane.visible };
    ...
    if (visibilityChanged) {
        if (firstPaneHidden) {
            ...
            nextSizes = [0, Math.max(MIN_PANE_SIZE, paneSizes[1] + paneSizes[0]), paneSizes[2]];
        } else if (thirdPaneHidden) {
            // Store hidden pane's size and add it to the middle pane
            previousPaneSizesRef.current.third = paneSizes[2];
            nextSizes = [paneSizes[0], Math.max(MIN_PANE_SIZE, paneSizes[1] + paneSizes[2]), 0];
            ...
            if (newMiddleSize < MIN_PANE_SIZE) {
                restoredFirstSize = Math.max(0, paneSizes[1] - MIN_PANE_SIZE); // Don't make first pane negative
            ...
            if (newMiddleSize < MIN_PANE_SIZE) {
                restoredThirdSize = Math.max(0, paneSizes[1] - MIN_PANE_SIZE); // Don't make third pane negative
                newMiddleSize = MIN_PANE_SIZE;
            }
        ...
        if (currentTotalSize === 0 || !currentVisibility.first && !currentVisibility.third) {
             ...
             if (visiblePanesCount === 1) { // Only middle visible
            ...
            nextSizes = paneSizes.map((size, index) => {
                const isVisible = (index === 0 && currentVisibility.first) || index === 1 || (index === 2 && currentVisibility.third);
    ...
    if (diff !== 0 && visibleIndices.length > 0) {
        ...
        if (diff !== 0) {
             ...
             } else if (!visibleIndices.includes(middleIndex) && visibleIndices.length > 0) {
         ...
         if (diff !== 0 && visibleIndices.length > 0) {
             ...
             const finalAdjustIndex = visibleIndices.includes(middleIndex) ? middleIndex : visibleIndices[visibleIndices.length - 1];
             nextSizes[finalAdjustIndex] = Math.max(MIN_PANE_SIZE, nextSizes[finalAdjustIndex] + diff);
         }
    ...
    // Only update state if sizes actually changed significantly (avoiding floating point noise)
    const sizesChanged = nextSizes.some((size, i) => Math.abs(size - paneSizes[i]) > 0.1);
    if (sizesChanged) {
  ...
  }, [containerWidth, firstPane.visible, thirdPane.visible, firstPane.preferredRatio, thirdPane.preferredRatio, paneSizes, onChange]); // Added preferredRatios as dependencies for fallback
  ...
  const getPaneStyle = useCallback((index: number, paneProps?: PaneProps): CSSProperties => ({
      ...
      {firstPane.visible && paneSizes[0] > 0 && (
        <EditorPaneResizer
          ...
          onTouchStart={(e: TouchEvent<HTMLSpanElement>) => {
             if (e.touches[0]) onResizeStart(1, e.touches[0].clientX); // Pass clientX
          }}
      ...
      {thirdPane.visible && paneSizes[2] > 0 && (
        <EditorPaneResizer
          ...
          onMouseDown={(e) => {
           ...
           onTouchStart={(e: TouchEvent<HTMLSpanElement>) => {
             if (e.touches[0]) onResizeStart(2, e.touches[0].clientX); // Pass clientX
           }}
          onTouchEnd={onResizeEnd}
          style={resizerProps?.style}
        />
