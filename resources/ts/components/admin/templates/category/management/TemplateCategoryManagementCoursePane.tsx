...
const TemplateCategoryManagementTemplatePane: React.FC = () => {
  ...
  const [tempTemplate, setTempTemplate] = useState<{ id: string; name: string } | null>(null);
  ...
  const handleAddNewTemplate = () => {
    ...
    setTimeout(() => {
      newTemplateRef.current?.scrollIntoView({ behavior: "smooth", block: "nearest" });
    }, 100);
  };
  ...
  const handleTemplateNameEdit = async (templateId: string, newName: string) => {
    ...
    try {
      updateTemplate(templateId, newName);
      return ""; // success
    } catch (error: any) {
              ...
              >
                ...
                >
                  ...
                  <EditableTypography
                    ...
                    value={template?.name ?? ""}
                    onSave={(newValue) => handleTemplateNameEdit(template.id, newValue)}
