...
import { templateRecipientGroupItems, templateRecipientGroups, students } from "../schema";
...
export namespace RecipientRepository {
  export const existsById = async (id: number): Promise<boolean> => {
    return (await db.$count(templateRecipientGroupItems, eq(templateRecipientGroupItems.id, id))) > 0;
  };

  export const existsByGroupIdAndStudentId = async (recipientGroupId: number, studentId: number): Promise<boolean> => {
    return (
      (await db.$count(
        templateRecipientGroupItems,
        and(eq(templateRecipientGroups.id, recipientGroupId), eq(students.id, studentId))
      )) > 0
    );
  };

  export const findById = async (id: number): Promise<Types.RecipientEntity | null> => {
  ...
  export const create = async (input: Types.RecipientCreateInput): Promise<Types.RecipientEntity> => {
    await RecipientGroupRepository.existsById(input.recipientGroupId).then(exists => {
      if (!exists) {
        throw new Error(`Recipient group with ID ${input.recipientGroupId} does not exist.`);
      }
    });
    ...
    await existsByGroupIdAndStudentId(input.recipientGroupId, input.studentId).then(exists => {
      if (exists) {
        throw new Error(
  ...
  // todo: transaction
  export const createList = async (input: Types.RecipientCreateListInput): Promise<Types.RecipientEntity[]> => {
    ...
    const existingRecipients = await db
      ...
      .where(
        and(
          eq(templateRecipientGroupItems.recipientGroupId, input.recipientGroupId),
          inArray(templateRecipientGroupItems.studentId, input.studentIds)
        )
      );

    const existingStudentIds = new Set(existingRecipients.map(r => r.studentId));
    ...
    try {
      const result = await db.insert(templateRecipientGroupItems).values(insertInput).returning();

      return result;
    } catch (error) {
      throw new Error(`Failed to create recipients: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  };

  export const deleteById = async (id: number): Promise<Types.RecipientEntity> => {
    const existingRecipient = await findById(id);
    if (!existingRecipient) {
      throw new Error(`Recipient with ID ${id} does not exist.`);
    }
    await db.delete(templateRecipientGroupItems).where(eq(templateRecipientGroupItems.id, id));
    return existingRecipient;
  };

  export const deleteSetByIds = async (ids: number[]): Promise<Types.RecipientEntity[]> => {
    ...
    try {
      ...
      throw new Error(`Failed to delete recipients: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  };

  export const findAllByGroupId = async (recipientGroupId: number): Promise<Types.RecipientEntity[]> => {
    await RecipientGroupRepository.existsById(recipientGroupId).then(exists => {
      if (!exists) {
        throw new Error(`Recipient group with ID ${recipientGroupId} does not exist.`);
      }
    });
  ...
  export const searchRecipientsInGroup = async (
    ...
    let baseQuery = db
      ...
      .from(templateRecipientGroupItems)
      .innerJoin(students, eq(students.id, templateRecipientGroupItems.studentId))
  ...
  export const findByStudentId = async (studentId: number): Promise<Types.RecipientEntity[]> => {
  ...
  export const loadForGroups = async (groupIds: number[]): Promise<Types.RecipientEntity[][]> => {
    if (groupIds.length === 0) return [];

    const allGroups = await Promise.all(groupIds.map(id => findAllByGroupId(id)));
    return allGroups;
  };

  export const countInGroup = async (groupId: number): Promise<number> => {
    return await db.$count(templateRecipientGroupItems, eq(templateRecipientGroupItems.recipientGroupId, groupId));
  };
}
