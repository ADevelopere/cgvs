...
import { CalendarTranslations } from "@/client/locale";
...
const DateScrollPicker: React.FC<DateScrollPickerProps> = ({
    ...
    const [anchorElMonth, setAnchorElMonth] = useState<null | HTMLElement>(
        null,
    );
    ...
    const handleDayInputBlur = () => {
        if (
            dayInputValue === "" ||
            !daysInMonth.includes(Number(dayInputValue))
        ) {
            setDayInputValue(
                calendar === Calendar.Gregorian
                    ? selectedDate.date().toString()
    ...
    const handleMonthInputBlur = () => {
        if (
            monthInputValue === "" ||
            !months.includes(Number(monthInputValue))
        ) {
            setMonthInputValue(
                calendar === Calendar.Gregorian
                    ? (selectedDate.month() + 1).toString()
