...
interface StorageTreeActions {
  ...
  updateTreeNode: (path: string, updater: (node: DirectoryTreeNode) => DirectoryTreeNode) => void;
...
export const useStorageTreeStore = create<StorageTreeState & StorageTreeActions>((set, get) => ({
  ...initialState,

  // Tree management actions
  setDirectoryTree: (tree) => set({ directoryTree: tree }),

  updateTreeNode: (path, updater) => set((state) => {
    const updateTreeNode = (nodes: DirectoryTreeNode[]): DirectoryTreeNode[] => {
  ...
  addChildToNode: (parentPath, children) => set((state) => {
    const updateTreeNode = (nodes: DirectoryTreeNode[]): DirectoryTreeNode[] => {
      return nodes.map((node) => {
  ...
  setPrefetchedNode: (path, isPrefetched) => set((state) => {
  ...
  // Queue management actions
  addToFetchQueue: (path) => set((state) => ({
    queueStates: {
      ...state.queueStates,
  ...
  removeFromFetchQueue: (path) => set((state) => ({
    queueStates: {
      ...state.queueStates,
      fetchQueue: new Set([...state.queueStates.fetchQueue].filter((p) => p !== path)),
    },
  })),

  addToExpansionQueue: (path) => set((state) => ({
    queueStates: {
      ...state.queueStates,
  ...
  removeFromExpansionQueue: (path) => set((state) => ({
    queueStates: {
      ...state.queueStates,
      expansionQueue: new Set(
  ...
  setCurrentlyFetching: (path, isFetching) => set((state) => {
