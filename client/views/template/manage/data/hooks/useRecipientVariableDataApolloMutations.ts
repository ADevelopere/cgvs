...
export const useRecipientVariableDataApolloMutations = () => {
  ...
  const [setValuesMutation] = useMutation(Document.setRecipientVariableValuesMutationDocument, {
    optimisticResponse: vars => {
      // Lightweight - only return the updated fields, no cache read
      const valuesArray = Array.isArray(vars.values) ? vars.values : [vars.values];
      ...
      const variableValues = valuesArray.reduce(
        ...
        {} as Record<number, string | null | undefined>
      );
    ...
    update(cache, { data }) {
      ...
      setTimeout(() => {
        try {
          ...
          const existingData = cache.readQuery<Graphql.RecipientVariableValuesByGroupQuery>({
          ...
          let found = false;
          const updatedData = existingData.recipientVariableValuesByGroup.data.map(recipient => {
            if (recipient.recipientGroupItemId === updatedRecipient.recipientGroupItemId) {
              found = true;
