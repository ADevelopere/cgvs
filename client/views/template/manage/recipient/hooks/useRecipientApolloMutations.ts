...
export const useRecipientApolloMutations = (templateId?: number) => {
  ...
  const [createMutation] = useMutation(Document.createRecipientMutationDocument, {
    update(cache, { data }) {
      ...
      if (templateId) {
        try {
          ...
          if (existingGroupsData?.templateRecipientGroupsByTemplateId) {
            const updatedGroups = existingGroupsData.templateRecipientGroupsByTemplateId.map(group =>
  ...
  const [createMultipleMutation] = useMutation(Document.createRecipientsMutationDocument, {
    update(cache, { data }) {
      if (!data?.createRecipients || data.createRecipients.length === 0) return;
          ...
          if (existingGroupsData?.templateRecipientGroupsByTemplateId) {
            const updatedGroups = existingGroupsData.templateRecipientGroupsByTemplateId.map(group =>
              group.id === recipientGroupId
                ? {
                    ...group,
                    studentCount: (group.studentCount || 0) + newRecipients.length,
                  }
                : group
            );
  ...
  const [deleteMutation] = useMutation(Document.deleteRecipientMutationDocument, {
    update(cache, { data }) {
          ...
          if (existingGroupsData?.templateRecipientGroupsByTemplateId) {
            const updatedGroups = existingGroupsData.templateRecipientGroupsByTemplateId.map(group =>
              group.id === recipientGroupId
                ? {
                    ...group,
                    studentCount: Math.max((group.studentCount || 0) - 1, 0),
                  }
                : group
            );
  ...
  const [deleteMultipleMutation] = useMutation(Document.deleteRecipientsMutationDocument, {
    update(cache, { data }) {
          ...
          if (existingGroupsData?.templateRecipientGroupsByTemplateId) {
            const updatedGroups = existingGroupsData.templateRecipientGroupsByTemplateId.map(group =>
              group.id === recipientGroupId
                ? {
                    ...group,
                    studentCount: Math.max((group.studentCount || 0) - deletedRecipients.length, 0),
                  }
                : group
            );
  ...
  return {
    createRecipient: (variables: Graphql.CreateRecipientMutationVariables) => createMutation({ variables }),
    createRecipients: (variables: Graphql.CreateRecipientsMutationVariables) => createMultipleMutation({ variables }),
    deleteRecipient: (variables: Graphql.DeleteRecipientMutationVariables) => deleteMutation({ variables }),
    deleteRecipients: (variables: Graphql.DeleteRecipientsMutationVariables) => deleteMultipleMutation({ variables }),
  };
};
