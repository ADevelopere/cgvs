...
import { TextElementNodeData } from "../reactFLow/nodeRenderer/TextElementNode";
import { ContainerNodeData, ElementBaseNodeData } from "../reactFLow/types";
...
import { ImageElementNodeData } from "../reactFLow/nodeRenderer/ImageNode";
...
function createTextNode(element: GQL.TextElement): Node<TextElementNodeData> | null {
  ...
  return {
    ...
    hidden: element.base.hidden ? true : false,
  };
}
...
export const NodesProvider: React.FC<{
    ...
    const initializeNodes = async () => {
      try {
        ...
        const hiddenNodesMap = initializedNodes
          ...
          .reduce((map, node) => {
            // Ensure hidden property is explicitly set to true
  ...
  const hideNode = React.useCallback((elementId: number) => {
    ...
    setNodesState(prev => {
      ...
      // Store in hidden nodes with hidden property set to true
