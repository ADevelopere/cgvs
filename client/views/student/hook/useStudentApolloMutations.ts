...
export const useStudentApolloMutations = () => {
    ...
    client.cache.modify({
      ...
      fields: {
        students(existingRef, { DELETE, storeFieldName }) {
          ...
          if (!storeFieldName.includes(JSON.stringify(currentParams.paginationArgs))) {
            return DELETE;
          }
  ...
  const [updateMutation] = useMutation(Document.partiallyUpdateStudentMutationDocument, {
    optimisticResponse: vars => {
      try {
        // Read existing student from cache using fragment
        const existingStudent = client.cache.readFragment<Graphql.StudentQuery["student"]>({
        ...
        const optimisticStudent = {
          ...
          id: vars.input.id,
          name: "name" in vars.input ? (vars.input.name ?? "") : (existingStudent?.name ?? ""),
          gender: "gender" in vars.input ? vars.input.gender : (existingStudent?.gender ?? null),
          nationality: "nationality" in vars.input ? vars.input.nationality : (existingStudent?.nationality ?? null),
          dateOfBirth: "dateOfBirth" in vars.input ? vars.input.dateOfBirth : (existingStudent?.dateOfBirth ?? null),
          email: "email" in vars.input ? vars.input.email : (existingStudent?.email ?? null),
          phoneNumber: "phoneNumber" in vars.input ? vars.input.phoneNumber : (existingStudent?.phoneNumber ?? null),
