...
import { useStudentGraphQL } from "./student.apollo";
...
export const StudentManagementProvider: React.FC<{
  ...
  const [pageInfo, setPageInfo] = useState<Graphql.PageInfo | null | undefined>(
    null,
  );
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      setLoading(true);
      try {
        const result = await studentsQuery(queryParams);
        if (result.data?.students?.data) {
          setStudents(result.data.students.data);
          setPageInfo(result.data.students.pageInfo);
        }
      } catch (error) {
