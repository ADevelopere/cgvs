...
export const TemplateGraphQLProvider: React.FC<{
 ...
 const [mutateCreate] = useMutation(Document.createTemplateQueryDocument, {
  update(cache, { data }) {
   ...
   const updatedCategories = existingData.templateCategories.map((category) => {
 ...
 const [mutateUpdate] = useMutation(Document.updateTemplateMutationDocument, {
  update(cache, { data }) {
   ...
   existingData.templateCategories.forEach((category) => {
    ...
    const found = category.templates.find((t) => t.id === updatedTemplate.id);
    if (found) {
   ...
   const updatedCategories = existingData.templateCategories.map((category) => {
    ...
    if (
     ...
     return {
      ...category,
      templates: templates.filter((t) => t.id !== updatedTemplate.id),
     };
    }
   ...
   update(cache, { data }) {
    ...
    const updatedCategories = existingData.templateCategories.map(
     (category) => {
      ...
      const templates: Graphql.Template[] = category.templates.filter(
       (template) => template.id !== suspendedTemplate.id,
      );
      return {
       ...category,
       templates: templates,
 ...
 const [mutateUnsuspendTemplate] = useMutation(
  Document.unsuspendTemplateMutationDocument,
  {
   update(cache, { data }) {
    ...
    let existingTemplate: Partial<Graphql.Template> | null | undefined = null;
