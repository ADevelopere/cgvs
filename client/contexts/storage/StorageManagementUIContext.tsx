...
export const StorageManagementUIProvider: React.FC<{
          ...
          if (isMounted && lastNavigationRequest.current === currentPath) {
            if (result) {
              ...
              if (result.items.length > 0) {
                if (
                  !focusedItem ||
                  !result.items.some((item) => item.path === focusedItem)
                ) {
  ...
  const selectRange = useCallback(
    (fromPath: string, toPath: string) => {
      ...
      const toIndex = currentItems.findIndex((item) => item.path === toPath);
  ...
  const pasteItems = useCallback(async (): Promise<boolean> => {
    ...
    if (clipboard.operation === "copy") {
      ...
      const success = await coreContext.copy(sourcePaths, destinationPath);
      ...
      const success = await coreContext.move(sourcePaths, destinationPath);
